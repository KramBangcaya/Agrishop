...
try {
    // Prepare the SQL query to count the total number of 'for delivery' orders for the given seller_id
    $sql = "
    SELECT COUNT(*) AS total_for_delivery
    FROM orders
    WHERE order_status = 'for delivery' AND seller_id = :seller_id;
    ";
    ...
    if ($result) {
        ...
        echo json_encode(["status" => "success", "data" => [], "message" => "No orders found for delivery."]);
    }
